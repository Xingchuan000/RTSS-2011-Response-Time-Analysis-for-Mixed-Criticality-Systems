...
def test_verified_surface_preserves_policy_failure_route_instead_of_bundle_invalid():
    registry = list(resolve_registry("protected_prefix").entries)
    closure = build_claim_closure(registry, "DEPLOYED_HI_SAFETY")
    failed_ids = {"DEPLOYED_POLICY_PRESERVATION", "FINAL_CLAIM_COMPOSITION"}

    certificates = {}
    for obligation_id in closure.verified_artifacts:
        failed = obligation_id in failed_ids
        certificates[obligation_id] = obligation_certificate(
            obligation_id=obligation_id,
            ...
            failure=(
                {
                    ...
                    "code": "RAW_TOP1_UNCHECKED_BREAKS_ENVELOPE",
                }
                if failed
                else None
            ),
