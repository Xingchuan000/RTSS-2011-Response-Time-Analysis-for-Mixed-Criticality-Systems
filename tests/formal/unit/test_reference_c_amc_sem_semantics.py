...
def test_abnormal_hi_uses_c_hi():
    assert release_class_and_budget(
        task=TASKSET["tasks"][0], mode_before_batch="LO", mode_after_batch="HI",
        abnormal_hi=True, switched_in_this_batch=True, primary_on_switch_time=True,
    ) == ("HI_ABNORMAL_SWITCH_TRIGGER", "HI", 5)


def test_same_batch_lo_release_is_primary():
    assert release_class_and_budget(
        task=TASKSET["tasks"][1], mode_before_batch="LO", mode_after_batch="HI",
        abnormal_hi=False, switched_in_this_batch=True, primary_on_switch_time=True,
    ) == ("LO_PRIMARY_SAME_BATCH_SWITCH_TIME", "HI", 3)


def test_later_hi_mode_lo_release_is_degraded():
    assert release_class_and_budget(
        task=TASKSET["tasks"][1], mode_before_batch="HI", mode_after_batch="HI",
        abnormal_hi=False, switched_in_this_batch=False, primary_on_switch_time=True,
    ) == ("LO_DEGRADED_HI_MODE", "HI", 1)


def test_hi_mode_hi_release_uses_c_hi():
    assert release_class_and_budget(
        task=TASKSET["tasks"][0], mode_before_batch="HI", mode_after_batch="HI",
        abnormal_hi=False, switched_in_this_batch=False, primary_on_switch_time=True,
    ) == ("HI_NORMAL", "HI", 5)
