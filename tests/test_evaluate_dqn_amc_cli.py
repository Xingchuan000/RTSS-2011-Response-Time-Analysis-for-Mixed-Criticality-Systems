...
def test_evaluate_cli_taskwise_v2_checkpoint_with_compat_flags(tmp_path: Path) -> None:
    """评估 CLI 应能接受 taskwise-v2 兼容参数并正常加载对应 checkpoint。"""

    env = {**os.environ, "KMP_DUPLICATE_LIB_OK": "TRUE"}
    train_dir = tmp_path / "taskwise_v2_train"
    eval_path = train_dir / "eval_taskwise_v2.csv"
    ...
    dqn_rows = [row for row in rows if row.get("method") == "dqn_agent"]
    assert dqn_rows
    assert all(int(row["action_count"]) == 25 for row in dqn_rows)
