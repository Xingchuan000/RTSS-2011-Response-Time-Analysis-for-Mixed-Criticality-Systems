"""Universal frozen-runtime -> V9.2 kernel conformance obligations.
...
queries over the V9.2 phase relation.  A conformance clause passes only when the
source theorem backend re-verifies and its corresponding V9.2 counterexample is
UNSAT.
"""
...
def prove_universal_conformance(
    ...
    *,
    ...
    formulas = (
        ("V9_2_PHASE_ORDER_TOTALITY", _phase_order_counterexample(model)),
        ("V9_2_P2_OBSERVE_ONLY", _p2_observe_only_counterexample(model)),
        ("V9_2_P3_RELEASE_SNAPSHOT", _p3_snapshot_counterexample(model)),
        ("V9_2_RELEASE_EFFECTIVE_DEMAND_CORRESPONDENCE",
         _release_effective_demand_counterexample(model)),
        ("V9_2_P7_TIME_AND_SERVICE", _p7_time_service_counterexample(model)),
        ("V9_2_FIRST_HI_MISS_REFLECTION", _hi_miss_reflection_counterexample(model)),
    )
    solver_rows = tuple(
