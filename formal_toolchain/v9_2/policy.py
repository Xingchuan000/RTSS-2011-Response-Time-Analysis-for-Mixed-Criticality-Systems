"""Exact V9.2 ranked FirstValid selection with a total explicit noop."""
...
def first_valid_explicit_noop(
    ...
    *,
    ...
    if action_dim <= 0 or len(ranking) != action_dim or len(valid_mask) != action_dim:
        raise ValueError("V9.2 ranking/mask dimension mismatch")
    normalized = tuple(int(value) for value in ranking)
    if tuple(sorted(normalized)) != tuple(range(action_dim)):
        raise ValueError("V9.2 ranking must be a complete action permutation")
    if not (0 <= noop_id < action_dim):
        raise ValueError("V9.2 explicit noop id is outside the action alphabet")
    if not bool(valid_mask[noop_id]):
        raise ValueError("V9.2 explicit noop must be valid in every controller state")
