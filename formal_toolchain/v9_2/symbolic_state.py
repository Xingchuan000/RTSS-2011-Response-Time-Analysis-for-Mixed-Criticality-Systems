"""Finite symbolic state for the V9.2 Policy--Timing Kernel.
...
class BoundModel:
    ...
    def from_bindings(cls, bindings: Mapping[str, Any], *, max_jobs_per_task: int = 2) -> "BoundModel":
        ...
        if len(noop_ids) != 1:
            raise ValueError("V9.2 requires exactly one explicit noop in bindings")
...
class SymbolicPolicyHistory:
    ...
    # obligation and therefore keeps the global V9.2 gate closed.
