"""Build the normative V10.1 machine bindings from the frozen request and current source."""
...
def _canonical_binding_value(value: Any) -> Any:
    ...
    if isinstance(value, float):
        if not math.isfinite(value):
            raise ValueError("V10.1 bindings cannot contain NaN/Inf")
...
def load_request(request_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("V10.1 proof request must be a JSON object")
    expected = {
    ...
    for key, value in expected.items():
        if data.get(key) != value:
            raise ValueError(f"V10.1 request {key} mismatch: {data.get(key)!r}")
    allowed = {
        "schema_version", "proof_route", "scope", "primary_claim",
        "target_id", "target_kind", "taskset_seed", "target_recipe",
        "tree_artifact_dir", "formal_inputs_dir", "tree_variant",
        "expected_tree_file_sha256", "source_binding",
    }
    unexpected = set(data) - allowed
    if unexpected:
        raise ValueError(f"V10.1 request contains unsupported fields: {sorted(unexpected)}")
    return data
...
def _workspace_root(request_path: Path) -> Path:
    ...
    if request_path.parent.name != "request":
        raise ValueError("V10.1 proof_request.json must live in <workspace>/request")
...
def _source_hashes(source_root: Path) -> dict[str, str]:
    ...
    for key, relative in files.items():
        ...
        if not path.is_file():
            raise ValueError(f"required V10.1 source binding missing: {relative}")
...
def _freeze_normalization_bounds(environment: Any, ordered_tasks: Any) -> dict[str, dict[str, int]]:
    ...
    The current V10.1 numeric proof models these bounds as exact binary64
    ...
    for task in ordered_tasks:
        ...
        if bound is None:
            raise ValueError(f"V10_1_P5_NORMALIZATION_BOUND_MISSING:{task.name}")
        lo = float(bound.min_cost)
        hi = float(bound.max_cost)
        if not (math.isfinite(lo) and math.isfinite(hi) and hi > lo):
            raise ValueError(f"V10_1_P5_NORMALIZATION_BOUND_INVALID:{task.name}")
        if not (lo.is_integer() and hi.is_integer()):
            raise ValueError(f"V10_1_P5_NONINTEGRAL_NORMALIZATION_BOUND_UNSUPPORTED:{task.name}")
        if abs(lo) >= 2**53 or abs(hi) >= 2**53:
            raise ValueError(f"V10_1_P5_NORMALIZATION_BOUND_EXCEEDS_EXACT_BINARY64_INTEGER_RANGE:{task.name}")
...
def _exact_integer_linear_constraints(adapter: Any, task_names: list[str]) -> list[dict[str, Any]]:
    ...
    if environment is None:
        raise ValueError("V10_1_P5_RUNTIME_ENVIRONMENT_UNBOUND")
    ...
    for task_name, constraint_name, rhs, coeff in checker._constraint_rows:
        ...
        if not rhs_f.is_integer():
            raise ValueError("V10_1_P5_SAFETY_RHS_NOT_EXACT_INTEGER")
        coeff_values = []
        for value in coeff.tolist():
            value_f = float(value)
            if not value_f.is_integer():
                raise ValueError("V10_1_P5_SAFETY_COEFFICIENT_NOT_EXACT_INTEGER")
            coeff_values.append(int(value_f))
        if len(coeff_values) != len(task_names):
            raise ValueError("V10_1_P5_SAFETY_ROW_DIMENSION_MISMATCH")
        if abs(int(rhs_f)) >= 2**53:
            raise ValueError("V10_1_P5_SAFETY_RHS_EXCEEDS_EXACT_BINARY64_INTEGER_RANGE")
        rows.append({
            "task": str(task_name),
    ...
    if not rows:
        raise ValueError("V10_1_P5_SAFETY_CHECKER_HAS_NO_ROWS")
    return rows
...
def _environment_domain(
    ...
    if rows_by_name and set(rows_by_name) != task_names:
        raise ValueError("V10_1_ACTUAL_DEMAND_BOUND_TASKSET_MISMATCH")
    ...
    for row in taskset["ordered_tasks"]:
        ...
        if lower < 1 or upper < lower:
            raise ValueError(f"V10_1_ACTUAL_DEMAND_BOUND_INVALID:{name}")
        if criticality == "HI" and upper > c_hi:
            raise ValueError(f"V10_1_HI_ACTUAL_DEMAND_EXCEEDS_C_HI:{name}")
        actual_demand = {
    ...
    return {
        "schema_version": "admissible_environment_domain_v10_1",
...
def _demand_semantics(runtime_config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "effective_demand_semantics_v10_1",
...
def build_bindings(request_path: Path, *, source_root: Path) -> dict[str, Any]:
    ...
    if str(_field(runtime_config, "semantics")) != "C_AMC_SEM":
        raise ValueError("V10_1_REQUIRES_C_AMC_SEM_RUNTIME_SEMANTICS")
    if bool(_field(runtime_config, "c_amc_sem_primary_on_switch_time")) is not True:
        raise ValueError("V10_1_REQUIRES_PRIMARY_ON_SWITCH_TIME_TRUE")
    if bool(_field(runtime_config, "drop_lo_jobs_on_hi_switch")) is not False:
        raise ValueError("V10_1_REQUIRES_C_AMC_SEM_ACTIVE_LO_CONTINUATION")
    if str(_field(runtime_config, "observation_mode")) != "v11_full_10d":
        raise ValueError("V10.1 currently binds only observation_mode=v11_full_10d")
    ...
    if len(noop_ids) != 1:
        raise ValueError("V10.1_REQUIRES_EXACTLY_ONE_EXPLICIT_NOOP")
    noop_id = noop_ids[0]
    adapter = target.runtime_adapter
    if adapter is None:
        raise ValueError("V10.1_REQUIRES_RUNTIME_ADAPTER")
    mask_contract = adapter.export_mask_contract()
    if mask_contract.get("selection") != "ranked_first_valid":
        raise ValueError("V10.1_REQUIRES_RANKED_FIRST_VALID")
    ...
    if any(bool(row.get("is_constraint_guided_pair")) or bool(row.get("is_residual_ranked")) for row in actions):
        raise ValueError("V10_1_P5_DYNAMIC_ACTION_SPACE_UNSUPPORTED")
    if bool(getattr(environment, "enable_residual_safety_fallback", False)):
        raise ValueError("V10_1_P5_RESIDUAL_SAFETY_FALLBACK_MUST_BE_DISABLED")
    if str(environment.budget_rounding_mode) != "ceil_floor":
        raise ValueError("V10_1_P5_ONLY_CEIL_FLOOR_ROUNDING_IS_BOUND")
    ...
    for row in safety_rows:
        ...
        if lhs_abs_bound >= 2**53:
            raise ValueError("V10_1_P5_SAFETY_LHS_EXCEEDS_EXACT_BINARY64_INTEGER_RANGE")
    env_domain = _environment_domain(
        taskset, target.provenance.get("actual_demand_bounds_by_task")
    )
    # V10.1 feature-transfer proves the executable EMA update in binary64 and
    # uses a 2x finite inductive envelope.  Keep all execution samples and that
    # envelope inside the exactly representable integer range used by the
    # source-bound float conversion.  Real task costs are far below this gate;
    # an exotic huge-cost taskset must be modelled explicitly instead.
    for row in env_domain["tasks"]:
        upper = int(row["actual_demand_domain"]["max"])
        if upper < 0 or 2 * upper >= 2**53:
            raise ValueError(
                f"V10_1_FEATURE_FP64_SAMPLE_DOMAIN_UNSUPPORTED:{row['task']}:{upper}"
            )
    ...
    event_binding = {
        "schema_version": "p0_event_order_binding_v10_1",
    ...
    closure = {
        "schema_version": "same_timestamp_closure_rank_v10_1",
        "rank": "mu(z)=7-p",
    ...
    for row in env_domain["tasks"]:
        ...
        history_domain_by_task[row["task"]] = {
            "recent_cost": {"min": 0, "max": sample_upper},
            "max_cost_k": {"min": 0, "max": sample_upper},
            "ema_cost_fp64_inductive": {"min": 0, "max": 2 * sample_upper},
            "overrun_ema_fp64_inductive": {"min": 0, "max": 2},
            "semantics": (
                "V10.1 policy-history domain: samples are bounded by runtime demand; "
                "EMA bounds are separately proved with exact IEEE-754 binary64 update semantics"
            ),
        }

    numeric = {
        "schema_version": "numeric_observation_binding_v10_1",
        ...
        "numeric_width_overflow": (
            "runtime samples and 2x FP64 EMA envelope are below 2^53; "
            "fixed-point output is explicitly clamped to the frozen integer domain"
        ),
    ...
    action_binding = {
        "schema_version": "policy_action_binding_v10_1",
    ...
    kernel_schema = {
        "schema_version": "policy_timing_kernel_state_v10_1",
    ...
    seed_task_binding = {
        "schema_version": "seed_task_binding_v10_1",
    ...
    environment_binding = {
        "schema_version": "environment_binding_v10_1",
        "domain": env_domain,
    ...
    bindings = {
        "schema_version": "v10_1_binding_bundle_v1",
        "proof_route": PROOF_ROUTE,
        "scope": SCOPE,
