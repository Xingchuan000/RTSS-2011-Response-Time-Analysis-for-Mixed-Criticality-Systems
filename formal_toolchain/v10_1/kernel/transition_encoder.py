"""Canonical eight-phase transition relation for the V10.1 kernel."""
...
def encode_p3_arrival_freeze(
    ...
    *, active_task_names: Collection[str] | None = None,
    ...
    if model.max_jobs_per_task < 2 and any(task.criticality == "LO" for task in model.tasks):
        raise ValueError("V10_1_TWO_SLOT_LO_CARRY_IN_REQUIRES_TWO_JOB_SLOTS")
...
def _history_domain(state: SymbolicKernelState, model: BoundModel) -> list[z3.BoolRef]:
    """Sound P5 history superset for the frozen binary64 runtime.

    Recent/max samples stay inside the concrete sample bound.  EMA values use
    the same deliberately widened 2x envelope discharged by
    ``v10_1.feature_transfer`` under exact IEEE-754 binary64 arithmetic.  The
    kernel never relies on these bounds as scheduler safety invariants; they
    only make the controller-history projection a finite runtime superset.
    """
...
def encode_p6_dispatch_case(
    ...
    *,
    ...
    if selected_task_name not in active:
        raise ValueError("V10_1_DISPATCH_CASE_INACTIVE_TASK")
    if selected_slot not in structural_slots(selected_task):
        raise ValueError("V10_1_DISPATCH_CASE_STRUCTURALLY_ABSENT_SLOT")
...
def encode_phase_step(
    ...
    *,
    ...
    raise ValueError(f"invalid V10.1 kernel phase: {phase}")
...
def encode_step(
    ...
    """The only V10.1 transition relation: exactly the canonical P0..P7 disjunction."""
