"""Trusted SAT classifier for verifier-regenerated V9.1 windows.
...
class DeployedRuntimeCounterexampleReplayer:
    ...
    The replayer does not call any V9.1 transition encoder.  It rebuilds the
...
def _window_demands(
    ...
    origin = _eval_int(solver_model, encoding.states[0].t)
...
def classify_sat_window(
    ...
    *,
    ...
    try:
        origin = _eval_int(sat_model, encoding.states[0].t)
        future = _window_demands(encoding, sat_model, model)
        z0_budgets = {
            task.name: _eval_int(sat_model, encoding.states[0].budgets[task.name])
            for task in model.tasks
        }
...
__all__ = ["DeployedRuntimeCounterexampleReplayer", "ReplayResult", "classify_sat_window"]
