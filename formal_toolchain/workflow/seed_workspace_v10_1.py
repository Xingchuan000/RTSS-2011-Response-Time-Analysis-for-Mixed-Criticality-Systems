"""Freeze a seed into the single, non-legacy V10.1 proof request surface."""
...
from formal_toolchain.v10_1.constants import PRIMARY_CLAIM, PROOF_ROUTE, REQUEST_SCHEMA, SCOPE
...
def _strict_target_manifest(seed_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if not path.is_file():
        raise ValueError("V10.1 requires formal_target_manifest.json")
    data = _read_json(path)
    if not isinstance(data, dict) or data.get("schema_version") != "formal_target_manifest_v1":
        raise ValueError("V10.1 only accepts formal_target_manifest_v1")
...
def freeze_seed_workspace_v10_1(
    seed_dir: Path,
    tree_variant: str,
    output_dir: Path,
    *,
    ...
    """Create a V10.1 workspace with the V10.1 BASE/PCSSC proof inputs."""
    ...
    for relative in ("request/inputs/tree_artifact", "request/inputs/formal_inputs", "verified", "logs"):
    ...
    if not formal_inputs.is_dir() or formal_inputs.is_symlink():
        raise UnresolvedInputError("AUTHORITATIVE_TARGET_MISSING", "V10.1 requires authoritative formal_inputs")
    ...
    if declared_seed is None:
        raise ValueError("V10.1 requires an explicit taskset_seed")
    ...
    (copied_inputs / "action_definitions_canonical.json").write_text(
        json.dumps({"schema_version": "action_definitions_canonical_v2_v10_1",
    ...
    (output_dir / "request/seed_artifact_inventory.json").write_text(
        json.dumps({"schema_version": "seed_artifact_inventory_v10_1", "tree_variant": tree_variant,
