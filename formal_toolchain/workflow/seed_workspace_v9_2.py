"""Freeze a seed into the single, non-legacy V9.2 proof request surface."""
...
from formal_toolchain.v9_2.constants import PRIMARY_CLAIM, PROOF_ROUTE, REQUEST_SCHEMA, SCOPE
...
def _strict_target_manifest(seed_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if not path.is_file():
        raise ValueError("V9.2 requires formal_target_manifest.json")
    data = _read_json(path)
    if not isinstance(data, dict) or data.get("schema_version") != "formal_target_manifest_v1":
        raise ValueError("V9.2 only accepts formal_target_manifest_v1")
...
def freeze_seed_workspace_v9_2(
    seed_dir: Path,
    tree_variant: str,
    output_dir: Path,
    *,
    ...
    """Create a V9.2 workspace with only the V9.2 Event proof inputs."""
    ...
    if not formal_inputs.is_dir() or formal_inputs.is_symlink():
        raise UnresolvedInputError("AUTHORITATIVE_TARGET_MISSING", "V9.2 requires authoritative formal_inputs")
    ...
    if declared_seed is None:
        raise ValueError("V9.2 requires an explicit taskset_seed")
    ...
    (copied_inputs / "action_definitions_canonical.json").write_text(
        json.dumps({"schema_version": "action_definitions_canonical_v2_v9_2",
    ...
    (output_dir / "request/seed_artifact_inventory.json").write_text(
        json.dumps({"schema_version": "seed_artifact_inventory_v9_2", "tree_variant": tree_variant,
